...
for workflow in workflows:
    ...
    # Do replacing. Make sure we only replace if not already replaced.
    if "KEYSTORE_FILE" not in content:
        if "run: ./gradlew assembleDebug" in content:
            content = content.replace("        run: ./gradlew assembleDebug", env_str_debug)
        elif "run: ./gradlew assembleRelease" in content:
            content = content.replace("        run: ./gradlew assembleRelease", env_str_release)
